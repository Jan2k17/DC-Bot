...
public class CMD_Coins extends ListenerAdapter {
    ...
    public void onSlashCommandInteraction(SlashCommandInteractionEvent e) {
        if (e.getName().equalsIgnoreCase("coins")) {
            ...
            e.getHook().editOriginal(sqlH.getTranslations("cmd_coins", sqlH.getGuildLang(e.getGuild().getId())).replace("%coins%", (sqlH.getCoins(e.getGuild().getId(), e.getMember().getId()))+"")).queue();
